...
static SemaphoreHandle_t hc06_tx_mutex;
...
void hc06_init_uart(void) {
    ...
    if (hc06_tx_mutex == NULL) {
        hc06_tx_mutex = xSemaphoreCreateMutex();
    }
}

void hc06_send_text(const char *text) {
    if (hc06_tx_mutex != NULL) {
        xSemaphoreTake(hc06_tx_mutex, portMAX_DELAY);
    }
    uart_puts(HC06_UART_ID, text);
    if (hc06_tx_mutex != NULL) {
        xSemaphoreGive(hc06_tx_mutex);
    }
}
